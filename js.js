const pokemones = [
    { id: 1, name: "Bulbasaur", img: "https://projectpokemon.org/images/normal-sprite/bulbasaur.gif", types: ["Grass", "Poison"], attack: 49, defense: 49, hp: 100 },
    { id: 2, name: "Ivysaur", img: "https://projectpokemon.org/images/normal-sprite/ivysaur.gif", types: ["Grass", "Poison"], attack: 62, defense: 63, hp: 100 },
    { id: 3, name: "Venusaur", img: "https://projectpokemon.org/images/normal-sprite/venusaur.gif", types: ["Grass", "Poison"], attack: 82, defense: 83, hp: 100 },
...
// Variables para control de la interfaz y el combate
// Variables para control de la interfaz y el combate
let jugadorPokemon = null;
let oponentePokemon = null;
let vidaJugador = 100;
let vidaOponente = 100;
let turnoJugador = true;
let ataqueEspecialDisponible = true;

// Elementos HTML
const pantallaSeleccion = document.getElementById("pantalla_seleccion");
const pantallaVs = document.getElementById("pantalla_vs");
const pantallaCombate = document.getElementById("pantalla_combate");
const listaPokemon = document.getElementById("pokemon-list");
const vidaJugadorTexto = document.getElementById("vida-jugador-texto");
const vidaOponenteTexto = document.getElementById("vida-oponente-texto");
const vidaJugadorBarra = document.getElementById("vida-jugador");
const vidaOponenteBarra = document.getElementById("vida-oponente");
...
document.getElementById('start-button').addEventListener('click', function() {
    ...
    document.getElementById('pantalla_seleccion').style.display = 'block';
});

// Generar lista de Pokémon
function generarListaPokemon() {
    ...
    pokemones.forEach((pokemon, index) => {
        ...
        // Agregar el evento de clic para seleccionar el Pokémon
        pokemonDiv.addEventListener('click', () => seleccionarPokemon(index));
...
function seleccionarPokemon(indice) {
    console.log("Seleccionado Pokémon:", pokemones[indice].name);
    // Asigna el Pokémon seleccionado y un oponente aleatorio
    jugadorPokemon = pokemones[indice];
    oponentePokemon = pokemones[Math.floor(Math.random() * pokemones.length)];

    // Configura la informacion del jugador
    document.getElementById("nombre-jugador").textContent = jugadorPokemon.name;
    document.getElementById("pokemon-jugador-img").src = jugadorPokemon.img;
    document.getElementById("tipo-jugador").textContent = `Tipo: ${jugadorPokemon.types.join(", ")}`;

    // Configura la informacion del oponente
    document.getElementById("nombre-oponente").textContent = oponentePokemon.name;
    document.getElementById("pokemon-oponente-img").src = oponentePokemon.img;
    document.getElementById("tipo-oponente").textContent = `Tipo: ${oponentePokemon.types.join(", ")}`;

    // Cambia la visibilidad de las pantallas
    document.getElementById("pantalla_seleccion").style.display = 'none';
    document.getElementById("pantalla_vs").style.display = 'block';
}

function iniciarCombate() {
    console.log("Iniciando combate..."); // Log para depuración

    // Ocultar la pantalla "vs" y mostrar la pantalla de combate
    document.getElementById("pantalla_vs").style.display = 'none';
    document.getElementById("pantalla_combate").style.display = 'block';

    // Asigna las imagenes de los Pokémon en combate
    document.getElementById("combate-jugador-img").src = jugadorPokemon.img;
    document.getElementById("combate-oponente-img").src = oponentePokemon.img;

    // Establecer la vida inicial a 100 
    vidaJugador = 100;
    vidaOponente = 100;

    // Mostrar vida y Pokémon en combate
    actualizarVida(vidaJugadorBarra, vidaJugadorTexto, vidaJugador);
    actualizarVida(vidaOponenteBarra, vidaOponenteTexto, vidaOponente);
}
// Actualizar barra de vida
function actualizarVida(barra, texto, vida) {
    const vidaRedondeada = Math.round(vida);
    barra.style.width = vidaRedondeada + "%";
    texto.textContent = `${vidaRedondeada}/100`;

    // Cambiar color de barra según vida
    if (vidaRedondeada <= 50) {
        barra.classList.add("low-health");
    } else {
        barra.classList.remove("low-health");
    }
}
...
function atacar() {
    ...
    vidaOponente -= jugadorPokemon.attack / 5; // Ajusta el divisor para variar el daño
    if (vidaOponente < 0) vidaOponente = 0;
    actualizarVida(vidaOponenteBarra, vidaOponenteTexto, vidaOponente);

    registrarMovimiento(`Tu Pokémon ${jugadorPokemon.name} ataca y causa daño a ${oponentePokemon.name}`);

    verificarResultado();
    if (vidaOponente > 0) {
        turnoJugador = false;
        setTimeout(respuestaMaquina, 2000); // Turno de la máquina
    }
}
...
function curarse() {
    ...
    registrarMovimiento(`Tu Pokémon ${jugadorPokemon.name} se cura y recupera 20 puntos de vida`);
...
function defender() {
    ...
    registrarMovimiento(`Tu Pokémon ${jugadorPokemon.name} se defiende, no recibe daño este turno`);
...
function ataqueEspecial() {
    if (!turnoJugador || !ataqueEspecialDisponible) return;

    vidaOponente -= vidaOponente * 0.5;
    if (vidaOponente < 0) vidaOponente = 0;
    actualizarVida(vidaOponenteBarra, vidaOponenteTexto, vidaOponente);

    registrarMovimiento(`Tu Pokémon ${jugadorPokemon.name} usa un ataque especial y reduce a la mitad la vida de ${oponentePokemon.name}`);

    ataqueEspecialDisponible = false;
    document.getElementById("boton-especial").disabled = true;
...
function respuestaMaquina() {
    if (vidaOponente > 0 && vidaJugador > 0) {
        mostrarTurno("Turno del oponente");
        
        setTimeout(() => {
            const accion = Math.floor(Math.random() * 5); 
            switch (accion) {
                case 0: 
                    vidaJugador -= oponentePokemon.attack / 5;
                    if (vidaJugador < 0) vidaJugador = 0;
                    registrarMovimiento(`El Pokémon oponente ${oponentePokemon.name} ataca con fuerza y causa daño a ${jugadorPokemon.name}`);
                    break;
                case 1: 
                    vidaJugador -= oponentePokemon.attack * 0.5;
                    if (vidaJugador < 0) vidaJugador = 0;
                    registrarMovimiento(`El Pokémon oponente ${oponentePokemon.name} realiza un ataque especial contra ${jugadorPokemon.name}`);
                    break;
                case 2: 
                    vidaOponente += 20;
                    oponentePokemon.defense += 20;
                    if (vidaOponente > 100) vidaOponente = 100;
                    registrarMovimiento(`El Pokémon oponente ${oponentePokemon.name} se cura y recupera vida`);
                    break;
                case 3: 
                    registrarMovimiento(`El Pokémon oponente ${oponentePokemon.name} se defiende, no recibe daño este turno`);
                    break;
                case 4:
                    vidaJugador -= oponentePokemon.attack / 30;
                    if (vidaJugador < 0) vidaJugador = 0;
                    registrarMovimiento(`El Pokémon oponente ${oponentePokemon.name} ataca a ${jugadorPokemon.name}`);
                    break;
            }
            ...
            if (vidaJugador > 0 && vidaOponente > 0) {
                turnoJugador = true;
                mostrarTurno("Turno de tu Pokémon");
            }
        }, 3000); 
    }
}
...
// Mostrar turnos
function mostrarTurno(mensaje) {
    const notificacion = document.getElementById("turno-notificacion");
...
function registrarMovimiento(movimiento) {
    ...
    nuevoMovimiento.textContent = movimiento;
    listaMovimientos.appendChild(nuevoMovimiento);

    // Guardar el movimiento en el localStorage
...
function borrarHistorial() {
    const listaMovimientos = document.getElementById("lista-movimientos");
    listaMovimientos.innerHTML = ""; // Limpia la lista en el DOM
    localStorage.removeItem("movimientos"); // Limpia el almacenamiento local
}
...
function verificarResultado() {
    if (vidaOponente <= 0) {
        alert(`¡Ganaste! ${jugadorPokemon.name} derrotó a ${oponentePokemon.name}`);
    } else if (vidaJugador <= 0) {
        alert(`¡Perdiste! ${oponentePokemon.name} derrotó a ${jugadorPokemon.name}`);
    }
}

function volverASeleccion() {
    console.log("Volviendo a la selección de Pokémon..."); // Log para depuración

    // Ocultar la pantalla de combate y mostrar la pantalla de seleccion
    document.getElementById("pantalla_combate").style.display = 'none';
    document.getElementById("pantalla_seleccion").style.display = 'block';
}
function cargarMovimientosDesdeLocalStorage() {
    const movimientosGuardados = JSON.parse(localStorage.getItem("movimientos")) || [];
    movimientosGuardados.forEach(movimiento => {
        const nuevoMovimiento = document.createElement("li");
        nuevoMovimiento.textContent = movimiento;
        listaMovimientos.appendChild(nuevoMovimiento);
    });
...
// Llamar a generar lista al cargar
generarListaPokemon();
